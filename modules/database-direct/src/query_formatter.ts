...
export default abstract class QueryFormatter {
  ...
  protected constructor(query: object, namespace: string) {
  ...
  protected formatTableName(tableName: string) {
    if (this.#namespace) return `${this.#namespace}.${tableName}`;
    return tableName;
  }
  ...
  protected formatFieldList(clause: unknown): string {
    if (clause) {
      if (typeof clause === "string") clause = [clause];
...
export class SelectQueryFormatter extends QueryFormatter {
  ...
  public constructor(query: object, namespace: string) {
