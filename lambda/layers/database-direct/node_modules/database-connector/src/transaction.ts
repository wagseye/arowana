...
export class BasicTransaction implements Transaction{
  ...
  public async start(): Promise<void> {
    ...
    try {
      ...
      await DatabaseConnector.query('START TRANSACTION');
      this.#isOpen = true;
    } catch(err) {
  ...
  public async commit(): Promise<void> {
    ...
    await DatabaseConnector.query('COMMIT TRANSACTION');
  }

  public async rollback(): Promise<void> {
    if (!this.isOpen()) return;
    console.log('Rolling back top-level transaction');
    await DatabaseConnector.query('ROLLBACK TRANSACTION');
  }
  ...
  public async start(): Promise<void> {
    if (!this.isOpen()) {
      ...
      try {
        ...
        await DatabaseConnector.query(`SAVEPOINT ${spName}`);
  ...
  public async commit(): Promise<void> {
    ...
    else
    {
      ...
      try {
        ...
        await DatabaseConnector.query(`RELEASE SAVEPOINT ${spName}`);
     } catch (err) {
        let msg = (err instanceof Error) ? err.message : 'Unknown error';
  ...
  public async rollback(): Promise<void> {
    ...
    else
    {
      ...
      try {
        ...
        await DatabaseConnector.query(`ROLLBACK TO SAVEPOINT ${spName}`);
       } catch (err) {
        let msg = (err instanceof Error) ? err.message : 'Unknown error';
