...
export class ConditionalExpression {
  ...
  public constructor(left: string | ConditionalExpression, operator: string, right: FieldType | ConditionalExpression) {
    this.left = left;
...
export abstract class Query<T extends DbObject> {
  ...
  protected async queryObjects(): Promise<T[]> {
    const dbRows: { [index: string]: any }[] = await Query.execute(this.toJSON());
...
export class SelectQuery<T extends DbObject> extends Query<T>{
  ...
  public toJSON() { return this.#queryObject };
}
...
export class UpdateQuery<T extends DbObject> extends Query<T>{
  ...
  public addRecord(rec: DbObject, changedFields: Set<string>) {
    ...
    const record = {};
    record["where"] = new ConditionalExpression(this.#queryObject['id'], "=", rec.id);
    const updates = {};
