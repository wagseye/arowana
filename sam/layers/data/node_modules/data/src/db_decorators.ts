...
export class DbObjectMapper {
  ...
  static addObject(obj: Object) {
    ...
    if (!objName) throw new Error('Object does not have name. It is probably not a class');
...
export class DbFieldMapper {
  static #fieldsByObjType: Map<object, Map<string, DbObjectField>> =
    new Map();

  static getFields(
    proto: object
  ): Map<string, DbObjectField> | undefined {
    if (!proto) {
...
export function dbObject(tableName: string, idPrefix: string): Function {
  ...
  return function (target: any, propertyKey: string, descriptor?: object) {
    ...
    fieldNames.forEach((fldName: string) => {
      ...
      if (!dbField) throw new Error(`Could not find field with name ${fldName}`);
    ...
    console.log(`dbOject constructor: ${inspect(Object.getPrototypeOf(Object.getPrototypeOf(target.constructor)))}`);
