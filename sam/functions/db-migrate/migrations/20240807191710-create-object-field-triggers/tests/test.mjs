...
export default class TestCreateObjectFieldTriggers {
  static async testNewTableForNewObjects() {
    ...
    res = await Database.runQuery(
      `SELECT count(*) as count FROM public.object1`
    );
    Test.assertEquals(0, parseInt(res[0].count));

    res = await Database.runQuery(
      `SELECT count(*) as count FROM public.object2`
    );
    Test.assertEquals(0, parseInt(res[0].count));
  }
  ...
  static async testExistingRecords() {
    const orgs = await Database.runQuery(
    ...
    const objs = await Database.runQuery(
    ...
    const seqs = await Database.runQuery(
    ...
    Test.assertEquivalent(222222222007, seq.increment);
    Test.assertEquivalent(2821109907437, seq.max);
    ...
    const allFlds = await Database.runQuery(
  ...
  static async testNewRecords() {
    let orgs = await Database.runQuery(
    ...
    let seqs = await Database.runQuery(
      `SELECT * FROM object_sequences WHERE organization_key='${org.id_key}'`
    );
    ...
    const flds = await Database.runQuery(
    ...
    let cols = await Database.runQuery(
